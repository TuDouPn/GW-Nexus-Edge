...
| G-06 | 公开 GitHub CI | 分支保护和完整 Actions 质量门禁已启用 | **PARTIAL**（2026-08-20，DEV-0005：PR #3 已合并为 `7d198fc`。main push：`Backend CI / Verify` SUCCESS、`Repository Security / Scan` SUCCESS、`Upload SARIF (main only)` SUCCESS、`Workflow Security / Lint` SUCCESS；**Dependency Submission FAILURE**（run 32356242289，`backend/backend/mvnw`）。依赖图 API 404。`Dependency Review / Review` 未设 Required。不得 PASS。Container Image Scan = NOT_APPLICABLE_YET） |
