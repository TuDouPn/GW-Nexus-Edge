...
| G-06 | 公开 GitHub CI | 分支保护和完整 Actions 质量门禁已启用 | **PASS**（2026-08-20，DEV-0005：PR #3 HEAD `398fd2e` 真实 Check Run 成功——`Backend CI / Verify`、`Repository Security / Scan`、`Workflow Security / Lint`；Fork 模型：`pull_request`、workflow `contents: read`、不读 Secret、无 `pull_request_target`、Lint 强制；main 保护 GET 复验 Required Checks 即为上述三 Job Display Name，strict、1 人评审、enforce_admins、禁 force push/删除。`Dependency Review / Review` 因依赖图 bootstrap 失败，**未**设为 Required。`Upload SARIF (main only)` 为 push-only，**未**设为 Required。Container Image Scan = NOT_APPLICABLE_YET） |
