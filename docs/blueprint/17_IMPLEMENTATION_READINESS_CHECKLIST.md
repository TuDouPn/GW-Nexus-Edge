...
| G-06 | 公开 GitHub CI | 分支保护和完整 Actions 质量门禁已启用 | **PARTIAL**（DEV-0005 实施中：G-06 仅在 PR CI 真实成功、Fork 安全模型检查通过、main 保护与 Required Checks（Job Display Name：`Backend CI / Verify`、`Repository Security / Scan`、`Workflow Security / Lint`）真实启用后才能 PASS。不得凭设计名称声称已启用。Container Image Scan = NOT_APPLICABLE_YET） |
